...
if __name__ == "__main__":
    ...
    SMPLX_FOLDER = "/home/hdh/project/KungFuAthleteBot/asserts/body_models"
    ...
    print("smplx_data keys:", smplx_data.keys())
    ...
    if args.save_path is not None:
        ...
        df.to_csv(
            args.save_path,
            index=False,    # 不保存行索引
            header=False,   # 不保存列名（纯数值列）
            float_format='%.8f'  # 保留8位小数，保证精度
        )
