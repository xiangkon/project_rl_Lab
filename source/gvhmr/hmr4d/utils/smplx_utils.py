...
def make_smplx(type="neu_fullpose", **kwargs):
    if type == "neu_fullpose":
        model = smplx.create(
            model_path="asserts/gvhmr_inputs/models/smplx/SMPLX_NEUTRAL.npz", use_pca=False, flat_hand_mean=True, **kwargs
        )
        ...
        model = BodyModelSMPLX(model_path="asserts/gvhmr_inputs/checkpoints/body_models", **bm_kwargs)
        ...
        model = BodyModelSMPLH(model_path="asserts/gvhmr_inputs/checkpoints/body_models", **bm_kwargs)

    elif type in ["smplx-circle", "smplx-groundlink"]:
        # don't use hand
        bm_kwargs = {
            "model_path": "asserts/gvhmr_inputs/checkpoints/body_models",
            "model_type": "smplx",
