...
class AMPRunner(OnPolicyRunner):
    ...
    def _construct_algorithm(self, obs: TensorDict) -> PPO:
        ...
        # -------------------------- 新增/修改部分 --------------------------
        # 从 alg_cfg 中移除 optimizer 参数（如果存在），避免传递给 PPOAMP
        unsupported_params = ["optimizer", "share_cnn_encoders"]
        for param in unsupported_params:
            if param in self.alg_cfg:
                self.alg_cfg.pop(param)  # 安全移除，不存在则无操作
        # ------------------------------------------------------------------
